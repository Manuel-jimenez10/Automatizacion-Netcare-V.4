"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = require("express");
const quote_followup_routes_1 = __importDefault(require("./quote-followup.routes"));
const quote_presented_routes_1 = __importDefault(require("./quote-presented.routes"));
const invoice_confirmed_routes_1 = __importDefault(require("./invoice-confirmed.routes"));
const invoice_reminder_routes_1 = __importDefault(require("./invoice-reminder.routes"));
const files_routes_1 = __importDefault(require("./files.routes"));
const whatsapp_routes_1 = __importDefault(require("./whatsapp.routes"));
const media_routes_1 = __importDefault(require("./media.routes"));
const whatsapp_template_routes_1 = __importDefault(require("./whatsapp-template.routes"));
const whatsapp_init_routes_1 = __importDefault(require("./whatsapp-init.routes"));
const xml_cache_routes_1 = __importDefault(require("./xml-cache.routes"));
const admin_notification_routes_1 = __importDefault(require("./admin-notification.routes"));
const router = (0, express_1.Router)();
router.use('/quotes', quote_followup_routes_1.default);
router.use('/quotes', quote_presented_routes_1.default);
router.use('/prefacturas', invoice_confirmed_routes_1.default);
router.use('/prefacturas', invoice_reminder_routes_1.default);
router.use('/files', files_routes_1.default);
router.use('/whatsapp', whatsapp_routes_1.default);
router.use('/media', media_routes_1.default);
router.use('/templates', whatsapp_template_routes_1.default);
router.use('/whatsapp-init', whatsapp_init_routes_1.default);
router.use('/xml-cache', xml_cache_routes_1.default);
router.use('/admin-notifications', admin_notification_routes_1.default);
exports.default = router;
