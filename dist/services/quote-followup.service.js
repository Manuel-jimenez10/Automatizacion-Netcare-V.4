...
const dailyBudgetUsed = () => (dailyBudget.date === todayKey() ? dailyBudget.used : 0);
/** 'YYYY-MM-DD HH:MM:SS' en UTC, el formato que espera EspoCRM. */
const toEspoDateTime = (date) => date.toISOString().slice(0, 19).replace('T', ' ');
class QuoteFollowUpService {
    constructor(deps = {}) {
        ...
        this.notifyExhausted =
            deps.notifyExhausted ||
                (params => admin_notification_service_1.adminNotificationService.notifyQuoteFollowUpExhausted(params));
    }
    get counterField() {
        return env_1.env.quoteFollowUpCounterField;
    }
    get maxAttempts() {
        return env_1.env.quoteFollowUpMaxAttempts;
    }
    // ─────────────────────────────────────────────────────────────
    // Proceso principal
    // ─────────────────────────────────────────────────────────────
    async processQuoteFollowUps(options = {}) {
        const dryRun = options.dryRun ?? env_1.env.quoteFollowUpDryRun;
        const result = {
            dryRun,
            totalPresented: 0,
            sent: 0,
        ...
        console.log(`   Campo contador: ${this.counterField} | Máx. seguimientos: ${this.maxAttempts} | Cada ${env_1.env.quoteFollowUpDays} días`);
        console.log('🚀 ============================================\n');
        if (!env_1.env.quoteFollowUpEnabled) {
            console.log('ℹ️ QUOTE_FOLLOWUP_ENABLED=false. Proceso desactivado.');
            return result;
        }
        if (!env_1.env.quoteFollowUpTemplateSid && !dryRun) {
        ...
        const maxPerRun = env_1.env.quoteFollowUpMaxPerRun;
        const maxPerDay = env_1.env.quoteFollowUpMaxPerDay;
        ...
        for (const quote of quotes) {
            ...
            if (!dryRun && this.attemptsThisRun > attemptsBefore && env_1.env.quoteFollowUpDelayMs > 0) {
                await sleep(env_1.env.quoteFollowUpDelayMs);
            }
        }
        this.printSummary(result);
        return result;
    }
    defer(result, quote, reason) {
        result.deferred++;
        result.details.push({
            quoteId: quote.id,
            quoteName: quote.name,
     ...
     */
    async assertCounterFieldExists(sampleQuote) {
        if (!sampleQuote)
            return;
        const quote = await this.espoCRMClient.getEntity('Quote', sampleQuote.id);
        if (!(this.counterField in quote)) {
            throw new CounterFieldMismatchError(`El campo "${this.counterField}" no existe en Quote (EspoCRM no lo devuelve al leer ` +
    ...
    // Una cotización
    // ─────────────────────────────────────────────────────────────
    async processQuote(listQuote, dryRun) {
        const base = { quoteId: listQuote.id, quoteName: listQuote.name, outcome: 'waiting' };
        ...
        if (daysElapsed < env_1.env.quoteFollowUpDays) {
            return { ...base, outcome: 'waiting', reason: `${daysElapsed}/${env_1.env.quoteFollowUpDays} días` };
        }
        console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        ...
        if (this.daysSinceReference(quote) < env_1.env.quoteFollowUpDays) {
        ...
        const recipientKey = (0, notification_phone_utils_1.phoneKey)(phone);
        ...
        try {
            twilioResponse = await this.sendTemplate({
                phone,
                contentSid: env_1.env.quoteFollowUpTemplateSid,
        ...
        if (attempt >= this.maxAttempts) {
            try {
                ...
                if (!adminNotified) {
                    console.warn(`   ⚠️ El aviso al administrador NO se entregó (${notifyResult?.reason || 'sin detalle'})`);
                }
            }
            catch (notifyError) {
     ...
     */
    async closeCycleOnTerminalError(quoteId, errorCode, attempt) {
        if (!TERMINAL_SEND_ERROR_CODES.has(Number(errorCode)))
            return;
        if (attempt >= this.maxAttempts)
            return;
        console.warn(`   🛑 Twilio devolvió ${errorCode} (destino no admite este mensaje). Se cierra el ciclo para no insistir.`);
        try {
     ...
     */
    async writeCounter(quoteId, value, extra = {}) {
        ...
        if (stored !== value) {
            throw new CounterFieldMismatchError(`El campo "${this.counterField}" no existe en Quote o no se pudo escribir ` +
    ...
    async reserveAttempt(quoteId, attempt) {
     ...
     */
    async clientHasReplied(quote, phone, contactId) {
        ...
        let twilio = 'unknown';
        if (env_1.env.quoteFollowUpCheckTwilio) {
        ...
        const twilioUnavailable = !env_1.env.quoteFollowUpCheckTwilio || twilio === 'unknown';
        if (crmFailed && twilioUnavailable) {
            throw new Error('no se pudo comprobar si el cliente respondió (EspoCRM y Twilio no respondieron). Se pospone al próximo día.');
        }
        console.log('      ↪ Sin respuestas');
        return false;
    }
    async hasInboundInCrm(phone, contactId, since) {
        try {
            ...
            return messages.some((m) => {
                if (contactId && m.contactId === contactId)
                    return true;
                return (0, notification_phone_utils_1.phoneKey)(m.name || '') === (0, notification_phone_utils_1.phoneKey)(phone);
            });
        }
        catch (error) {
    ...
    async hasInboundInTwilio(phone, since) {
        try {
    ...
    lastFollowUpDate(quote) {
        return this.referenceDate(quote).date;
    }
    daysSinceReference(quote) {
    ...
    // Registro y resumen
    // ─────────────────────────────────────────────────────────────
    async logMessageInEspo(quote, phone, twilioResponse, attempt) {
        try {
