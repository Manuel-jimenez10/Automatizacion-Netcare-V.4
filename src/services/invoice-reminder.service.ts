...
import { sendPrefacturaReminderMessage } from './twilio.service';
...
export class InvoiceReminderService {
   ...
   */
  async processReminders(testMode: boolean = false): Promise<void> {
    console.log('\n🚀 ============================================');
    console.log('🚀 Iniciando proceso de Recordatorios de Prefacturas');
    console.log(`🚀 Fuente: Reporte ID ${REPORT_ID}`);
    if (testMode) console.log('🧪 MODO PRUEBA: Solo se enviará a billing contact "prueba seguimiento"');
    console.log('🚀 ============================================\n');
   ...
   */
  private async getInvoiceIdsFromReport(): Promise<string[]> {
   ...
   */
  private async processInvoiceReminder(invoice: EspoCRMInvoice, testMode: boolean = false): Promise<'sent' | 'skipped'> {
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    ...
    if (diffDays === 3 && !invoice.recordatorio3DiasEnviado) {
       ...
       templateSidToUse = env.prefacturaReminder3DaysSid;
       ...
       templateSidToUse = env.prefacturaReminderOverdueSid;
       isOverdue = true;
    } 
    else {
       let reason = `Días restantes: ${diffDays}. No aplica ninguna regla.`;
       if (diffDays === 3 && invoice.recordatorio3DiasEnviado) reason = `Ya se envió recordatorio de 3 días el ${invoice.recordatorio3DiasEnviado}`;
       if (diffDays <= 0 && invoice.avisoVencimientoEnviado) reason = `Ya se envió aviso de vencimiento el ${invoice.avisoVencimientoEnviado}`;
    ...
    const pdfUrl = `${env.publicUrl}/api/files/${invoice.prefacturaAdjuntaId}`;
    console.log(`📎 PDF ID: ${invoice.prefacturaAdjuntaId}`);
    console.log(`📎 PDF URL: ${pdfUrl}`);

    // 4. Enviar mensaje por Twilio
    console.log('📱 Enviando WhatsApp...');
    const twilioResponse = await sendPrefacturaReminderMessage({
    ...
    if (isReminder3Days) updatePayload.recordatorio3DiasEnviado = nowDateTime;
    if (isOverdue) updatePayload.avisoVencimientoEnviado = nowDateTime;
    ...
    try {
      ...
      if (conversations.length > 0) {
        ...
        if (invoice.billingContactName) {
           ...
           if (invoice.billingContactId) conversationPayload.contactId = invoice.billingContactId;
        }
